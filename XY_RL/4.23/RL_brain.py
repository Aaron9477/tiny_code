...
class DeepQNetwork:
    def __init__(
        ...
        self.next_action_true_reward = np.array([0,0,0])   # 定义下一步的奖励值
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('target_net'):
            ...
            with tf.variable_scope('l4'):
                w3 = tf.get_variable('w4', [n_l1, self.n_actions], initializer=w_initializer, collections=c_names)
                b3 = tf.get_variable('b4', [1, self.n_actions], initializer=b_initializer, collections=c_names)
                # l3 = tf.nn.relu(tf.matmul(self.s, w3) + b3)
                self.q_next = tf.matmul(l3, w4) + b4


    def store_transition(self, s, a, r, s_,next_reward):
        ...
        self.next_action_true_reward = next_reward
    ...
    def choose_action(self, observation):
        ...
        if np.random.uniform() < self.epsilon:  # random decide whether explore randomly
            ...
            print('real reward:' + str(self.next_action_true_reward))
            print('q值:' + str(actions_value))
            actions_value = self.next_action_true_reward + actions_value
                # actions_value = np.array([self.next_action_true_reward[0] + actions_value[0][0],self.next_action_true_reward[1] + actions_value[0][1],self.next_action_true_reward[2] + actions_value[0][2]])
            print('更新后:' + str(actions_value))
            action = np.argmax(actions_value)   # argmax返回最大数字的索引
            print(action)
        else:
    ...
    def learn(self):
        ...
        eval_act_index = batch_memory[:, self.n_features].astype(int)   # 找到对应的列截图
