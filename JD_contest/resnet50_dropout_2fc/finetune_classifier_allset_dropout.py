...
######################
    ...
    class Net_classifier(nn.Module):
        def __init__(self):
            ...
            self.fc2= torch.nn.Linear(2048, 30) 
        ...
        def forward(self, x):
            ...
            #x = self.fc3(x)
            return x
    Net_classifier=Net_classifier().cuda()
...
#####################
    ...
    optimizer = torch.optim.Adam(model.Net_classifier.parameters(), args.lr,
                                weight_decay=args.weight_decay)
#######################
        ...
        if i % args.print_freq == 0:
            ...
            f_train.writelines('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Prec@5 {top5.val:.3f} ({top5.avg:.3f}) \n'.format(
                   epoch, i, len(train_loader), batch_time=batch_time,
                   data_time=data_time, loss=losses, top1=top1, top5=top5))
        if float(i)/float(len(train_loader)) > 0.5: # half epoch
            break
