...
##---the different of it with fix_refine.py isthat it will generate five--##
import os
import argparse
...
def main(div_rate, src_path, save_train_path, save_val_path, interval, i):
    for pig in range(1,31):
        ...
        images_all=range(1, num_image+1, interval)
        num_val = int(num_image / interval * (1 - div_rate)) + 1
        # print(num_val)
        # print(num_image)
        images_group = list_of_groups(images_all,num_val)
        images_group[4].append(2951)    # the last one after zip will be one group, so we need append and pop.why?????????
        images_group.pop()
