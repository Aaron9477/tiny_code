...
def judge_num_larger(remain_list, input_list, threshold):   #find all the data larger than threshold
    list_over_threshold = []
...
def judge_num_smaller(remain_list, input_list, threshold):   #find all the data smaller than threshold
    list_over_threshold = []
    for i in remain_list:
        if input_list[i][2] < threshold:
            # remain_list.remove(i)
...
def process_data_larger(input):   # process data
    # print(input)
    remain_list = range(30) # all the 30 list, waiting removing
    list_over_09 = judge_num_larger(remain_list, input, 0.9)
    ...
    list_over_08 = judge_num_larger(remain_list, input, 0.8)
    if len(list_over_08) > 0:   #0.8
        ...
        for a in list_over_08:
            ...
            remain_list.remove(a)
            remain_possible -= 0.7
        list_over_01 = judge_num_larger(remain_list, input, 0.1)
    ...
    list_over_07 = judge_num_larger(remain_list, input, 0.7)
    if len(list_over_07) > 0:   #0.7
        ...
        list_over_02 = judge_num_larger(remain_list, input, 0.2)
    ...
    list_over_06 = judge_num_larger(remain_list, input, 0.6)
    if len(list_over_06) > 0:   #0.7
        ...
        list_over_03 = judge_num_larger(remain_list, input, 0.3)
    ...
    list_over_05 = judge_num_larger(remain_list, input, 0.5)
    ...
    list_over_04 = judge_num_larger(remain_list, input, 0.4)
...
def process_data_smaller(input, threshold):
    remain_list = range(30)
    list_over_01 = judge_num_larger(remain_list, input, 0.1)
    if len(list_over_01) > 0:
        list_small = judge_num_smaller(remain_list, input, threshold)
        num_change = 0
        for a in list_small:
            num_change += (threshold - input[a][2])
            input[a][2] = threshold
            remain_list.remove(a)
        every_change = round(num_change/len(list_over_01), 6)
        for b in list_over_01:
            input[b][2] -= every_change
        return input
    else:
        for i in remain_list:
            input[i][2] = round(1.0/len(remain_list), 6)
...
def main():
    ...
    while(True):
        try:
            ...
            # processed_data = process_data_larger(thirty_pigs)

            processed_data = process_data_smaller(thirty_pigs, smaller_threshold)
